...
void x11_shavite512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
...
__host__
void hex_shavite512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	const uint32_t threadsperblock = TPB;
	...
	// note: 128 threads minimum are required to init the shared memory array
	x11_shavite512_gpu_hash_64 << <grid, block >> >(threads, startNounce, (uint64_t*)d_hash, d_nonceVector, d_filter, filter_val);
	//MyStreamSynchronize(NULL, order, thr_id);
}
