...
__global__
void quark_keccak512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
...
__global__
void quark_keccak512_gpu_hash_64_v30(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
...
__host__
void hex_keccak512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	const uint32_t threadsperblock = 256;
	...
	if (device_sm[dev_id] >= 320)
		quark_keccak512_gpu_hash_64 << <grid, block >> >(threads, startNounce, (uint64_t*)d_hash, d_nonceVector, d_filter, filter_val);
	else
		quark_keccak512_gpu_hash_64_v30 << <grid, block >> >(threads, startNounce, (uint64_t*)d_hash, d_nonceVector, d_filter, filter_val);
