...
void quark_blake512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint32_t *g_nonceVector, uint64_t *g_hash, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
...
void quark_blake512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_outputHash, int order)
{
	...
	if (device_sm[dev_id] >= 500 && cuda_arch[dev_id] >= 500)
		quark_blake512_cpu_hash_64_sp(threads, startNounce, d_nonceVector, d_outputHash, NULL, 0);
	else
#endif
	{
		...
		quark_blake512_gpu_hash_64<<<grid, block>>>(threads, startNounce, d_nonceVector, (uint64_t*)d_outputHash, NULL, 0);
	}
...
// hex filtered
__host__
void hex_blake512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_outputHash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
#ifdef SP_KERNEL
	int dev_id = device_map[thr_id];
	if (device_sm[dev_id] >= 500 && cuda_arch[dev_id] >= 500)
		quark_blake512_cpu_hash_64_sp(threads, startNounce, d_nonceVector, d_outputHash, d_filter, filter_val);
	else
#endif
	{
		const uint32_t threadsperblock = 256;
		dim3 grid((threads + threadsperblock - 1) / threadsperblock);
		dim3 block(threadsperblock);
		quark_blake512_gpu_hash_64 << <grid, block >> >(threads, startNounce, d_nonceVector, (uint64_t*)d_outputHash, d_filter, filter_val);
	}
	MyStreamSynchronize(NULL, order, thr_id);
}
