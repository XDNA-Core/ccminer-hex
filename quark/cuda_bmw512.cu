...
#endif
void quark_bmw512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
...
__host__
void hex_bmw512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	const uint32_t threadsperblock = 32;
	dim3 grid((threads + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);
	...
	int dev_id = device_map[thr_id];
	if (device_sm[dev_id] > 300 && cuda_arch[dev_id] > 300)
		quark_bmw512_gpu_hash_64 << <grid, block >> >(threads, startNounce, (uint64_t*)d_hash, d_nonceVector, d_filter, filter_val);
	else
		quark_bmw512_gpu_hash_64_30 << <grid, block >> >(threads, startNounce, (uint64_t*)d_hash, d_nonceVector, d_filter, filter_val);
}
