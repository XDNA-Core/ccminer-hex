...
void quark_groestl512_gpu_hash_64_quad(const uint32_t threads, const uint32_t startNounce, uint32_t * g_hash, uint32_t * __restrict g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
#if __CUDA_ARCH__ >= 300
...
__host__
void hex_groestl512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	uint32_t threadsperblock = TPB;
	...
	if (device_sm[dev_id] >= 300 && cuda_arch[dev_id] >= 300)
		quark_groestl512_gpu_hash_64_quad << <grid, block >> >(threads, startNounce, d_hash, d_nonceVector, d_filter, filter_val);
	else
		quark_groestl512_sm20_hash_64(thr_id, threads, startNounce, d_nonceVector, d_hash, order, d_filter, filter_val);
}
