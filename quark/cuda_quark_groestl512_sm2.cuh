...
__global__
void quark_groestl512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint32_t *g_hash, uint32_t *g_nonceVector, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
#if __CUDA_ARCH__ < 300 || defined(_DEBUG)
...
__host__
void quark_groestl512_sm20_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, uint32_t* const d_filter = NULL, const uint32_t filter_val = 0)
{
	...
	quark_groestl512_gpu_hash_64<<<grid, block>>>(threads, startNounce, d_hash, d_nonceVector, d_filter, filter_val);
}
